...
import { fetchQrPng, renderQrLabelSvg } from "@/lib/mark-render";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const qr = await fetchQrPng(result.entity.profileUrl, 620);
    const svg = renderQrLabelSvg({
    ...
    return new NextResponse(svg, {
      headers: {
        ...
        "Content-Disposition": `${download ? "attachment" : "inline"}; filename="craftid-qr-label-${result.entity.craftId}.svg"`,
        "Cache-Control": "private, max-age=0, must-revalidate",
        "X-Robots-Tag": "noindex",
      },
    });
  } catch (error) {
    console.error("CraftID QR label generation failed", error);
    return new NextResponse("QR label generation unavailable", { status: 502 });
  }
}
